...
//导出部分数据
	public function export(){
		if (Request::instance()->get()) {
			...
			$info = DB::name('XwtMedia')->where('status=1')->alias('m')->field('m.id,m.title,m.url,m.case,m.cost_price,m.guide_price,m.in_type_id,m.in_effect_id,m.ch_type_id,m.media_type_id,m.media_from_id,m.gate_id,m.remark,m.create_time')->where($where)->paginate(10)->each(function($item,$key){
				$data1 = DB::name('XwtMediaType')->field('id,title')->where('id',$item['in_type_id'])->find();
				$item['in_type_id'] = $data1['title'];    	//halt($item);//收录类型
				$data2 = DB::name('XwtMediaType')->field('id,title')->where('id',$item['in_effect_id'])->find();
				$item['in_effect_id'] = $data2['title'];	//dump($data2);//收录效果
